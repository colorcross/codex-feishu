...
### 修复
- 群聊 @ 门禁现在会校验被 @ 的对象是否为机器人自己。配置 `feishu.bot_open_ids` 后，群里 `@其他人` 不再触发 Feique；同时兼容 Feishu 将白名单群误报为非 `group` 的事件。
- Feishu webhook / long-connection 的白名单判断改为读取运行时最新配置，避免配置热更新后仍使用启动快照。
- 主动 retry-loop 告警不再对“高频但全部成功”的运行报“可能需要换个思路”。现在必须同时满足运行次数阈值和失败次数阈值，才会提示重试循环。

### 文档
- 补充群聊通用配置说明：`security.require_group_mentions` 控制是否需要 @，`feishu.bot_open_ids` 控制 @ 的对象必须是机器人自己。
