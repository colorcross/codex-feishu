  ...
  it('uses native Feishu reply when reply_quote_user_message is enabled', async () => {
    const setup = await createService({
      service: {
        ...
        memory_pin_age_basis: 'updated_at', team_digest_enabled: false, team_digest_interval_hours: 24, team_digest_chat_ids: [], intent_classifier_enabled: false, intent_classifier_timeout_ms: 5000, intent_classifier_min_confidence: 0.8,
      },
    });
    runCodexTurnMock.mockResolvedValue({
      sessionId: 'thread-1',
  ...
  const base: BridgeConfig = {
    ...
    projects: {
      default: {
        ...
        wiki_space_ids: [],
        admin_chat_ids: [],
        notification_chat_ids: [], mcp_servers: [], skills: [],
        run_priority: 100,
